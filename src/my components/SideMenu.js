        ...
        <div className="p-4 mt-20 flex flex-col space-y-4">
          {menuItems.map((item) => (
            ...
            <Link
              ...
              to={item.path}
              onClick={toggleMenu}
              // Conditionally apply the lg:hidden class only to the "Login" item
              className={`text-left px-4 py-3 bg-gray-200 hover:bg-gray-300 rounded-lg font-semibold transition-colors duration-300 shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${item.name === 'Login' ? 'lg:hidden' : ''}`}
            >
              {item.name}
            </Link>
          ))}
